...
while True:
    ...
    print("Press 2 to export data for devices from the CSV file .")
    dev_type = input("Please enter 1 or 2 and press Enter: ")
    if dev_type == '1':
        break
    elif dev_type == '2':
        break
...
def get_csv_dev_profile_data(address, username, password, profile, devlist, from_time, to_time):
    m_values_kwh = []
    m_values_kvah = []
    for device_id in devlist:
        url = f"{address}/api/1/devices/{device_id}/profiles/{profile}/entries?from={from_time}&to={to_time}"
        try:
            r = requests.get(url, auth=(username, password), verify=False)
            r.raise_for_status()
